...
export function SwapConfirmationDialog({
  ...
  const displayFromAmount = actualAmountIn || fromAmount
  const displayToAmount = toAmount
  const displayMinAmountOut = actualMinAmountOut
  const displaySlippage = actualSlippage

  const fromAmountNum = Number.parseFloat(displayFromAmount) || 0
  const toAmountNum = Number.parseFloat(displayToAmount) || 0
  const minAmountOutNum = displayMinAmountOut ? Number.parseFloat(displayMinAmountOut) : 0
          ...
          <div className="flex flex-col items-center space-y-4 sm:space-y-6">
            <div className="flex items-center justify-center w-full bg-card/50 rounded-xl p-3 sm:p-5 border border-border/50 shadow-sm">
              <div className="text-center flex-1 min-w-0 px-2">
                ...
                <p className="text-xs sm:text-sm font-medium text-primary truncate">{fromToken?.symbol || "Unknown"}</p>
              </div>
              ...
              <div className="text-center flex-1 min-w-0 px-2">
                ...
                <p className="text-sm sm:text-lg lg:text-xl font-bold text-primary break-all leading-tight">
                  {toAmountNum > 0 ? formatCurrency(toAmountNum) : "0"}
                </p>
                <p className="text-xs sm:text-sm font-medium text-primary truncate">{toToken?.symbol || "Unknown"}</p>
              </div>
            </div>
            ...
            <div className="w-full rounded-xl bg-card/50 p-3 sm:p-5 text-xs sm:text-sm space-y-2 sm:space-y-3 border border-border/50 shadow-sm">
              ...
              <div className="flex justify-between py-1 gap-2">
                <span className="text-muted-foreground flex-shrink-0">Exchange Rate</span>
                <span className="font-medium text-right break-all min-w-0">
                  {exchangeRate > 0 ? formatCurrency(exchangeRate) : "0"} {toToken?.symbol || "Token"}
                </span>
              </div>
              {displaySlippage !== undefined && (
                <div className="flex justify-between py-1 gap-2">
                  <span className="text-muted-foreground flex-shrink-0">Slippage Used</span>
                  <span className="font-medium text-right">{displaySlippage}%</span>
                </div>
              )}
              {displayMinAmountOut && (
                <div className="flex justify-between py-1 gap-2">
                  <span className="text-muted-foreground flex-shrink-0">Minimum Received</span>
                  <span className="font-medium text-right break-all min-w-0">
                    {minAmountOutNum > 0 ? formatCurrency(minAmountOutNum) : "0"} {toToken?.symbol || "Token"}
                  </span>
                </div>
              )}
              <div className="flex justify-between py-1 gap-2">
                <span className="text-muted-foreground flex-shrink-0">Protocol Fee (0.3%)</span>
                <span className="font-medium break-all min-w-0 text-right">
