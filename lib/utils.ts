...
export function calculatePriceImpact(inputAmount: number, outputAmount: number, exchangeRate: number): number {
  if (exchangeRate <= 0 || inputAmount <= 0) return 0
  const expectedOutput = inputAmount * exchangeRate
  if (expectedOutput <= 0) return 0
  return Math.max(0, ((expectedOutput - outputAmount) / expectedOutput) * 100)
}

export function calculateMinimumReceived(amount: number, slippagePercentage: number): number {
  return amount * (1 - slippagePercentage / 100)
}
